...
public class VideoLogsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_video_logs);
        getSupportActionBar().setTitle(R.string.video_call_logs);
    ...
    void inflateMessages(Query query){
        FirebaseRecyclerOptions<VideoCall> options = new FirebaseRecyclerOptions.Builder<VideoCall>()
                .setQuery(query, VideoCall.class)
                .build();

        final FirebaseRecyclerAdapter FBRA = new FirebaseRecyclerAdapter<VideoCall, VoiceCallHolder>(options) {
            @Override
            protected void onBindViewHolder(@NonNull final VoiceCallHolder holder, int position, @NonNull VideoCall model) {
                holder.setName(model.getContactUserName());
                holder.setContact(model.getContactInfo());
                holder.setLogIcon(model.getType());
                holder.setTimeDuration(model.getTimestamp());
...
//                                    });
                ...
                viewChat.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        startActivity(new Intent(VideoLogsActivity.this, ChatActivity.class));
                    }
                });
                viewProfile.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        startActivity(new Intent(VideoLogsActivity.this, JyotishProfilesActivity.class));
                    }
                });
                viewVideoCall.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        startActivity(new Intent(VideoLogsActivity.this, VidCallActivity.class));
                    }
                });
                viewVoiceCall.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        startActivity(new Intent(VideoLogsActivity.this, VoiceCallActivity.class));
                    }
                });
            }
            ...
            public VoiceCallHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.video_log_card, parent, false);
                return new VoiceCallHolder(itemView);
            }

        };
        FBRA.startListening();
        logs.setAdapter(FBRA);
    }
    ...
    public class VoiceCallHolder extends RecyclerView.ViewHolder{
        ...
        void setTimeDuration(long timestampp){
